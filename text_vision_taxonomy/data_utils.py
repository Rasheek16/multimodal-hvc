...
from .segment_frame_utils import sample_segment_frames_from_dir
...
def load_frame_tensor(
    ...
    if start_sec is not None and end_sec is not None and not frame_dir_is_segment:
        paths = sample_segment_frames_from_dir(
            Path(frame_dir),
            start_sec=float(start_sec),
            end_sec=float(end_sec),
            num_frames=int(num_frames or cfg.num_frames),
            fps=fps,
        )
    else:
...
class VideoTextFrameDataset(Dataset):
    def __init__(
        ...
        self.frame = frame.reset_index(drop=True) if "segment_id" in frame.columns else video_level_manifest(frame).reset_index(drop=True)
        self.cfg = cfg
        self.teacher_cache = dict(teacher_cache or {})
        self.text_embedding_dim = int(text_embedding_dim)
        self.train = bool(train)
    ...
    def _frame_paths(self, source_video_id: str, frame_dir: Path) -> List[Path]:
        cache_key = f"{source_video_id}|{frame_dir}"
        cached = self._frame_path_cache.get(cache_key)
        if cached is not None:
            return cached
        paths = list_frame_paths(frame_dir)
        self._frame_path_cache[cache_key] = paths
        return paths
