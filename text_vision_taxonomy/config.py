...
def _default_text_vision_root(project_root: Path) -> Path:
    module_root = Path(__file__).resolve().parent
    if (module_root / "data").exists() and (module_root / "checkpoints").exists():
        return module_root
    return project_root / "text_vision"


def _default_text_model_path(text_vision_root: Path, project_root: Path) -> Path:
    local_checkpoint = text_vision_root / "roberta_base_finetuned_dualhead.pt"
    if local_checkpoint.exists():
        return local_checkpoint
    return project_root / "text_classification" / "roberta_base_finetuned_dualhead.pt"


PROJECT_ROOT = Path(os.environ.get("HVC_PROJECT_ROOT", _default_project_root())).resolve()
WORKSPACE_ROOT = _default_workspace_root()
TEXT_VISION_ROOT = Path(os.environ.get("HVC_TEXT_VISION_ROOT", _default_text_vision_root(PROJECT_ROOT))).resolve()
DATASET_ROOT = Path(os.environ.get("HVC_DATASET_ROOT", WORKSPACE_ROOT / "datasets" / "diversified")).resolve()
TEXT_MODEL_PATH = Path(
    os.environ.get("HVC_TEXT_MODEL_PATH", _default_text_model_path(TEXT_VISION_ROOT, PROJECT_ROOT))
).resolve()
...
class TextVisionConfig:
    ...
    taxonomy_labels: List[str] = field(default_factory=lambda: list(TAXONOMY_LABELS))
    num_taxonomy_labels: int = len(TAXONOMY_LABELS)
